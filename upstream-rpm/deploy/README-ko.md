# RHEL 8 x86_64 프록시 다운로드 및 설치 — 20260925-5

[이번 묶음의 수정 내용과 검증](NEXT-WAVE-20260925-5.md),
[배포 파일](https://github.com/emotionbug/slop/releases/tag/linuxoss-install-20260925-5).
...
mkdir -p linuxoss-update-20260925-5
cd linuxoss-update-20260925-5
BASE='https://github.com/emotionbug/slop/releases/download/linuxoss-install-20260925-5'
for FILE in linuxoss-install-20260925-5.tar.gz linuxoss-install-20260925-5.tar.gz.sha256; do
  wget -e use_proxy=yes -e https_proxy="$PROXY" -e http_proxy="$PROXY" \
    --timeout=60 --tries=3 -O "$FILE" "$BASE/$FILE"
done
sha256sum -c linuxoss-install-20260925-5.tar.gz.sha256
tar -xzf linuxoss-install-20260925-5.tar.gz
cd linuxoss-install-20260925-5
...
SSH/PAM 및 스토리지·네트워크 핵심 서비스의 자체 빌드 전환은 이번 적용 대상이 아닙니다.
