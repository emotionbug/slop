...
Trivy 0.74.0 실제 검사에서 자체 RPM 52개는 목록/SBOM에는 식별됐지만,
Vendor 기준으로 `third-party`로 분류되어 기본 RHEL 취약점 검사에서 모두 제외됐습니다.
`comprehensive` 옵션에서도 같았고, CycloneDX 재검사에서는 공급자 분류가 사라져
Red Hat 피드의 결과가 붙었습니다. 이를 자체 RPM의 수정 판정으로 사용하지 않습니다.
[검사 결과와 연계 방법](TRIVY-INTEGRATION.md)에 재현 결과와 누락 CSV 도구를 정리했습니다.
수정 근거, 실제 설치 버전, 실행 중 바이너리 및 서비스 검증을 함께 확인해야 합니다.
