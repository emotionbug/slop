%global optflags -fPIE -O2 -g -gdwarf-4 -fstack-protector-strong -fcf-protection
%global __global_ldflags -Wl,-z,relro,-z,now
%global with_test_subpackage 1

Name:           sblim-cmpi-base
Version:        1.6.4
Release: 34.linuxoss%{?dist}
Summary:        SBLIM CMPI Base Providers

License:        EPL-1.0
URL:            https://sourceforge.net/projects/sblim/
Source0: sblim-cmpi-base-1.6.4.tar.bz2
...
%setup -q
libtoolize --force
ACLOCAL_PATH=/usr/share/aclocal autoreconf --install --force
...
%patch9 -p1 -b .fix-possible-null-dereference
%patch10 -p1 -b .gcc15-fixes
%patch11 -p1 -b .image-mode
%patch12 -p1 -b .no-wc-ldflag
%patch13 -p1 -b .secure-tmpfiles
...
%check
# Load the produced libraries without connecting to a CIM server.
LD_LIBRARY_PATH=%{buildroot}%{_libdir} /usr/bin/python3.11 - <<'PY'
import ctypes,glob
paths=glob.glob('%{buildroot}%{_libdir}/cmpi/*.so')
assert paths
for path in paths: ctypes.CDLL(path)
print('CMPI ELF load checks:',len(paths))
PY
