...
Release: %{glibcrelease}.40.linuxoss.1
...
Source9998: linuxoss-prepared-sources.sha256
Patch9999: glibc-el8-20260926.patch
...
##############################################################################
%prep
%if 0%{?linuxoss_resume}
%setup -q -T -D -n %{glibcsrcdir}
%else
...
if ! readelf -h /usr/bin/true >/dev/null 2>&1; then
  true_command='/usr/bin/coreutils --coreutils-prog=true'
fi

# Show the auxiliary vector as seen by the new library
# (even if we do not perform the valgrind test).
LD_SHOW_AUXV=1 $run_ldso $true_command
