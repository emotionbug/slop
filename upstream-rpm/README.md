...
[커널 전용 키트](kernel-deploy/README-ko.md)는 Linux 7.2.7 기반 커널과 동일 버전
개발용 RPM을 기존 커널 옆에 추가합니다. Release 3에서 EL8의 cgroup v1 메모리
제어를 복구하고 BTF와 기존 드라이버 구성을 유지합니다. 기존 커널을 기본값으로
보존하고 한 번 부팅한 뒤 명시적으로 확정하는 절차를 제공합니다.
SSH용 `boot-once-remote`는 패닉·dracut 실패 시 재부팅 설정과 20분 확정 제한시간을
추가합니다. 미확정이면 현재 실행 중이던 커널로 돌아가며, 완전 정지에 대한
하이퍼바이저 watchdog을 대신하지는 않습니다.
Trivy의 커널 RPM 누락과 upstream 버전 접미사 중복 평가도 보완했습니다.
이 커널은 Red Hat 서명·지원 커널이 아니며 실제 서버 서비스 확인은 별도입니다.

## 사용자 공간 설치 묶음: 20260925-6
...
커널은 위의 별도 설치 키트로 진행합니다. glibc·systemd·SSH/PAM은 보안 백포트와
부팅·인증 의존성 보완이 남은 후속 작업입니다.
