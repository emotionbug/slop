...
전체 323종 중 **51종과 의존 RPM 2종**을 담은 [평가용 묶음](CANDIDATES.md)을
공개했습니다. 바이너리 RPM 53개와 대응 SRPM 38개, 체크섬, 읽기 전용 사전 검사
도구를 포함한 105.5 MiB 파일입니다. 대상 24종에서 46종으로 확대한 뒤
coreutils 2종, libsolv, protobuf-c, jbig2dec-libs를 추가했습니다.

이전 48 RPM의 UBI 8 설치·DNF 검사와 Java 8 폰트/JPEG, Python, 로컬 SSH/TLS 등을
포함한 실행 검사 15개가 통과했습니다. 추가 5 RPM을 그 환경에 설치하고 최종 DNF 검사와
추가 실행 검사 4개도 통과했습니다. 실제 서버 헤더와 비교한 이름 기반 의존성
누락과 downgrade는 0개이며 파일/rich dependency 1,183건은 미판정입니다.
pkgconf는 SONAME 변경, mtr은 패킷 크기 검사 실패로 이번 묶음에서 보류했습니다.

**11종은 제거된 라이브러리 심볼에 대한 서버 소비 프로그램 확인이 남아 있습니다.**
묶음의 `preflight-candidates.sh`가 144개 심볼 참조와 `dnf --assumeno` 결과를 한 번에
수집합니다. 패키지를 설치하지 않습니다. 실제 서버 적용 및 전체 CVE 해결 완료를
의미하지 않으며 커널·GCC·시스템 glibc/OpenSSL은 이번 묶음에 포함하지 않았습니다.
