...
def main():
    ...
    if validation.get('_combined', {}).get('runtime_result') != 'passed':
        raise SystemExit('Combined container validation has not passed')
    dest = ROOT/'output'/('candidate-bundle-'+VERSION)
    dest.mkdir(exist_ok=True)
    manifest = {'status': 'local-evaluation-candidates', 'rpm_signed': False,
    ...
    (dest/'README.txt').write_text(
        'EL8 local RPM evaluation candidates: rsync 3.5.1, zlib 1.3.2, PCRE2 10.48.\n'
        'Seven replacement RPMs and three corresponding source RPMs.\n'
        'Not a complete 323-package upgrade; not tested on the target server.\n'
        'RPMs are unsigned local builds, not Red Hat official packages.\n'
